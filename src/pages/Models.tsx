...
export default function Models() {
  ...
  if (error) {
    return (
      <div className="p-4">
        <Card className="border-destructive-text/30">
          <CardContent className="flex flex-col items-center gap-3 py-10 text-center" role="alert">
            <AlertCircle className="h-8 w-8 text-destructive-text" aria-hidden="true" />
            <div className="space-y-1">
              <p className="text-sm font-medium text-destructive-text">Failed to load models</p>
      ...
      {models.length === 0 ? (
        <Card className="min-h-0 flex-1">
          <CardContent className="flex h-full flex-col items-center justify-center gap-3 py-16 text-center">
            <Boxes aria-hidden="true" className="h-10 w-10 text-muted-foreground/30" />
            ...
            <Table>
              <TableHeader className="sticky top-0 z-10 bg-card">
                <TableRow>
                  ...
                  <TableHead className="h-9 w-[110px] px-3 text-[11px] uppercase tracking-wider">Alias</TableHead>
                  <TableHead className="h-9 w-[90px] px-3 text-right text-[11px] uppercase tracking-wider">Context</TableHead>
                  <TableHead className="h-9 w-[86px] px-3 text-[11px] uppercase tracking-wider">Vision</TableHead>
                  <TableHead className="h-9 w-[88px] px-3 text-[11px] uppercase tracking-wider">Status</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {models.map((model) => (
                  <TableRow key={model.id}>
                      ...
                      <Badge
                        ...
                        className={
                          model.supports_vision
                            ? "h-5 gap-1 rounded px-1.5 text-[10px] text-foreground"
                    ...
                    <TableCell className="px-3 py-2">
                      <Badge
                        ...
                        className={
                          model.visible
                            ? "h-5 gap-1 rounded px-1.5 text-[10px] border-success/25 bg-success/10 text-success"
                            : "h-5 gap-1 rounded px-1.5 text-[10px] text-muted-foreground"
                        }
                      >
